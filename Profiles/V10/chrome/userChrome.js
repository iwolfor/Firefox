...
(function(){
  ...
  var UCJS      = new Array("UCJSFiles"); //UCJS Loader 仕様を適用 (NoScriptでfile:///を許可しておく)
  var arrSubdir = new Array("", "Bugfix", "SubScript", "TabMixPlus", "UCJSFiles", "UserContent","UserMenu");    //スクリプトはこの順番で実行される
  //===================================================================
  const ALWAYSEXECUTE   = ['rebuild_userChrome.uc.xul', 'rebuild_userChrome.uc.js', 'UC脚本管理器.uc.js']; //制运行的脚本，无视禁用列表
  var INFO = true;
  ...
  window.userChrome_js = {
    ...
    get hackVersion () {
      ...
      if (!/^(chrome:\/\/\S+\/content\/)\S+/i.test( Error().fileName) ){
        ...
        return this.hackVersion = "0.8+";
      } else if (Error().fileName.indexOf("chrome://browser/content/browser.xhtml -> ") == 0) {
...
this.debug('Parsing getScripts: '+((new Date()).getTime()-Start) +'msec');
    ...
    load: function (){
        ...
        if (!doc) doc = document;
        if (!(doc.constructor.name === 'XULDocument'))
          return 0;
        var observer = {
